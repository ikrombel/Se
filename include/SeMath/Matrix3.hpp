...
public:
    ...
    void SetDiagonal(const Vector3 &v) {
        m00_ = v.x_; m01_ = 0.0f; m02_ = 0.0f;
        m10_ = 0.0f; m11_ = v.y_; m12_ = 0.0f;
        m20_ = 0.0f; m21_ = 0.0f; m22_ = v.z_;
    }
    ...
    static Matrix3 &Orthonormalize(Matrix3 &ret, const Matrix3 &m);
    ...
    String ToString() const {
        return cformat("%g %g %g %g %g %g %g %g %g", m00_, m01_, m02_, m10_, m11_, m12_, m20_, m21_, m22_);
    }
...
inline Matrix3 &Matrix3::Orthonormalize(Matrix3 &ret, const Matrix3 &m) {
    Vector3 x = Vector3(m.m00_, m.m10_, m.m20_);
    Vector3 y = Vector3(m.m01_, m.m11_, m.m21_);
    Vector3 z = x.CrossProduct(y);
    y = z.CrossProduct(x);
    x.Normalize();
    y.Normalize();
    z.Normalize();
    ret.m00_ = x.x_; ret.m01_ = y.x_; ret.m02_ = z.x_;
    ret.m10_ = x.y_; ret.m11_ = y.y_; ret.m12_ = z.y_;
    ret.m20_ = x.z_; ret.m21_ = y.z_; ret.m22_ = z.z_;
    return ret;
}
