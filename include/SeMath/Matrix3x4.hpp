...
public:
    ...
    bool Equals(const Matrix3x4& rhs, float eps = M_EPSILON) const
    {
        ...
        for (unsigned i = 0; i < 12; ++i)
        {
            if (!Se::Equals(leftData[i], rightData[i], eps))
                return false;
        }
    ...
    String ToString() const {
        return cformat("%g %g %g %g %g %g %g %g %g %g %g %g", 
                m00_, m01_, m02_, m03_, m10_, m11_, m12_, m13_, m20_, m21_, m22_, m23_);
    }
...
inline Matrix4 Matrix4::operator *(const Matrix3x4& rhs) const
{
    return Matrix4(
        m00_ * rhs.m00_ + m01_ * rhs.m10_ + m02_ * rhs.m20_,
        m00_ * rhs.m01_ + m01_ * rhs.m11_ + m02_ * rhs.m21_,
