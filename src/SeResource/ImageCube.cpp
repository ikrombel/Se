...
Color ImageCube::SampleNearest(const Vector3& direction) const
{
    ...
    const IntVector2& texel = projection.second_;
    return faceImages_[projection.first_]->GetPixel(texel.x_, texel.y_);
}
...
std::pair<CubeMapFace, Vector2> ImageCube::ProjectDirectionOnFace(const Vector3& direction)
{
    ...
    result.second_.x_ = result.second_.x_ * 0.5f + 0.5f;
    result.second_.y_ = result.second_.y_ * 0.5f + 0.5f;
    return result;
}
